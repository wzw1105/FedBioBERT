...
def _get_wikitext(conf, split="train", filename=None):
    if split == "train":
        # BertSingle/RobertaSingleModel
        tokenizer = RobertaTokenizer.from_pretrained(f'{conf.project_path}/TokenizerModel', max_len=conf.max_len)
        train_dataset = RobertaDataset(filename, tokenizer=tokenizer)
        return train_dataset
    else:
        raise Exception("only for train dataset implemented!!!")
...
def get_dataset(
    ...
    if name == "cifar10" or name == "cifar100":
        ...
        return _get_wikitext(conf, split, 'wiki-train-clear-no-use.txt')
    elif name == "wikitext-pubmed":
        file_id = conf.graph.comm_round % conf.tot_files if conf.graph.comm_round % conf.tot_files != 0 else conf.tot_files
        filename = conf.data_dir + f"/pubmed_{file_id}.txt"
        conf.logger.log(f"rank - {conf.graph.rank} use file {filename} !!!")
        return _get_wikitext(conf, split, filename)
    elif name== "biobert-pubmed":
        return _get_wikitext(conf, split, 'BioBertPubMed.txt')
    else:
