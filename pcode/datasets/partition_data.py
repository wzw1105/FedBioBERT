...
class Partition(object):
    ...
    def __init__(self, data, indices):
        self.data = data
        #self.indices = indices[start_indice : start_indice + batch_size]
...
class DataPartitioner(object):
    ...
    def __init__(
        ...
        self.partitions = []  # partitions是一个存储每个client的数据indices的，其中每个元素是一个list（该client的数据下标）

        # get data, data_size, indices of the data.
        self.data_size = len(data)
        if type(data) is not Partition:
            self.data = data
            indices = np.array([x for x in range(0, self.data_size)])  # indices就是存储下标的array
        else:
            self.data = data.data
    ...
    def _get_consistent_indices(self, indices):
        if dist.is_initialized():
            ...
            # self.conf.logger.log(f"{'Master' if self.conf.graph.rank ==0 else 'Client' } broadcast/receive the partition indices.")
            indices = torch.IntTensor(indices)
            dist.broadcast(indices, src=0)   # 这里做了分发操作，分发的实际上是所有client的数据信息。
            return list(indices)
        else:
            return indices

    def use(self, partition_ind):
        return Partition(self.data, self.partitions[partition_ind]) # partitions[partition_ind]就是client=partition_id的client的所有数据下标
